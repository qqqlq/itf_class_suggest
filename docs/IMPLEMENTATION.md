...
| フィールド | 説明 |
|---|---|
| `name` | グループ名（専門科目 / 専門基礎科目 / 基礎科目（共通） / 基礎科目（関連） / 教職課程）|
...
**基礎科目（関連）の科目番号判定ロジック修正 + 教職課程グループ追加**

`data/curricula/mast.json` のみの変更。

**問題の発見:**
- 科目番号がアルファベット始まり（`FBA*`, `YA*`, `YB*` 等）→ 筑波大各学群・学類の科目
- 科目番号が数字始まり（`8xxxxxxx`）→ 自由科目（特設）
- 科目番号 `990xxxx` → 博物館に関する科目

この対応関係が未実装のまま、以下の問題があった:
- 「他学群又は他学類の授業科目」: `prefixes` 未定義のため何もマッチしていなかった
- 「博物館に関する科目・自由科目(特設)」: `prefixes: ["82","Y","F"]` で人文・文化学群（`F*`）・芸術/体育専門学群（`Y*`）を誤って捕捉していた

**修正内容:**

| カテゴリ | 変更前 | 変更後 |
|---|---|---|
| 他学群又は他学類の授業科目 | prefixes未定義（常に不マッチ） | `A`〜`Z` の26文字プレフィックス、`GA/GB/GC/GE` を excludePrefixes で除外 |
| 博物館に関する科目・自由科目(特設) | `["82","Y","F"]` | `["8","990"]`（`8xxxxxxx` 全件＋博物館科目 `990xxxx`） |
| 教職課程科目（新規） | なし | `"9"` 始まり・`"990"` を除外（卒業要件外、minCredits:0） |

**補足:**
- `9xxxxxxx` のうち `990xxxx` は博物館科目、`900〜989xxxx` は教職課程科目と判明
- 教職課程は卒業要件外（minCredits: 0）だが、取得者のCSVで「その他・未分類」に埋もれないよう明示的に分類
